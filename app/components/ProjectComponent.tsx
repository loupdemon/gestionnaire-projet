...
const ProjectComponent: FC<ProjectProps> = ({ project, admin, style, onDelete }) => {
    ...
    //alert pour demander confirmation de suppresion de projet
    ...
    const tasksByStatus = project.tasks?.reduce(
        (acc, task) => {
            if (task.status === "To Do") acc.toDo++; // à chaque tache trouvé il augmente le compteur
            else if (task.status === "In Progress") acc.inProgress++;
            else if (task.status === "Done") acc.done++;
            return acc //ensemble du tableau des valeurs
        },
        {
            toDo: 0, inProgress: 0, done: 0
        } // s'il n'y a pas de tache , il revient un tableau vide
    ) ?? { toDo: 0, inProgress: 0, done: 0 }

    const progressPercentage = totalTasks ? Math.round((tasksByStatus.done / totalTasks) * 100) : 0 //pourcentage de total des taches en progress
    ...
    //fonctiond e copie de code d'invitation
    const handleCopyCode = async () => {
        try {
            if (project.inviteCode) {
                await navigator.clipboard.writeText(project.inviteCode);//on fait appel à une fct navigateur clipboard
    ...
    return (
        <div key={project.id} className={`${style ? 'border border-base-300 p-5 shadow-sm ' : ''}text-base-content rounded-xl w-full text-left`}>
            ...
            <div className='flex'>
                ...
                {admin === 1 && (
                    <button className='btn btn-sm ml-3 bg-red-300 hover:bg-red-400 text-white' onClick={handleDeleteClick}>
                        <Trash className='w-4' />
                    </button>
                )}
            </div>
