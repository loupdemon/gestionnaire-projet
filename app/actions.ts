...
export async function getProjectsCreatedByUser(email: string) {
    try {
        const projects = await prisma.project.findMany({
            //comme on cherche plusieur projets on mets many, si c'était un on mettrait findunique
            where: {
                createdBy: { email },
            },
            include: {
                tasks: {
                    // on cherche les tâches associés au projet
                    include: {
                        // pour chaque tache on recupere l'utilisateur et proprietaire du projet
                        user: true,
                        createdBy: true,
                    },
                },
                users: {
                    //on récupere aussi la lsite de tous les utilisateurs associés à ce proejt
                    select: {
                        user: {
                            select: {
                                id: true,
...
//fonction de récupération de projet par id avec ses taches
